...
class TrajHeadWithGoalAbs(nn.Module):
    ...
    """
    def __init__(self, d: int, T_pred: int, d_goal: Optional[int] = None, hidden_mult: int = 2):
        super().__init__()
        self.T_pred = T_pred
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(fuse_in, hidden_mult * d), nn.GELU(),
            nn.Linear(hidden_mult * d, 2 * T_pred),
        )
...
class DownstreamTrajPredictor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.head = TrajHeadWithGoalAbs(d=d, T_pred=T_pred, d_goal=d_goal, hidden_mult=hidden_mult)
